...
void HeapInit(Heap *ph) { ph->numOfData = 0; }
int isEmpty(Heap *ph) {
    if (ph->numOfData == 0)
        return True;
    else
...
int getHighPriorityIdx(Heap *ph, int idx) {
    ...
    {
        if (ph->heapArr[getLeftChildIdx(idx)] -
                ph->heapArr[getRightChildIdx(idx)] >
            0)
            return getLeftChildIdx(idx);
        else
...
int HeapDel(Heap *ph) {
    ...
    while (childIdx = getHighPriorityIdx(ph, parentIdx)) {
        if (lastData - ph->heapArr[childIdx] >= 0) break;
...
int main() {
    ...
    while (1) {
        ...
        else if (cal == 'd') {  // 힙에서 삭제 루트노드 반환
            printf("%d\n", HeapDel(&heap));
        }

        else if (cal == 'p') {  // 힙을 레벨 순서로 출력
            printHeap(&heap);
        }

        else if (cal == 'q') {  // 프로그램 종료
            break;
        }
    }
